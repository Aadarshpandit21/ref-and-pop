...
const getParticularBooks = async function(req,res){
      let particular = await bookModel.find({ $and :[{authorName : "Chetan Bhagat" },{ stockAvailble: true } ]})
      res.send({msg:particular})
 }
const getXINRBooks = async function(req,res){
    let getBook = await bookModel.find({prices:{"indianPrice" : "1000"}})
    res.send({msg:getBook});
}
const getRandomBooks = async function(req,res){
    let randomBook = await bookModel.find({totalPages:{$gt:500}})
